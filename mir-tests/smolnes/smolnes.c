...
int main(int argc, char **argv) {
  size_t nread = SDL_RWread(SDL_RWFromFile(argv[1], "rb"), rombuf, 1024 * 1024, 1);
...
loop:
  ...
  for (tmp = cycles * 3 + 6; tmp--;) {
    if (ppumask & 24) { // If background or sprites are enabled.
      //printf("background or sprites enabled.\n");
      if (scany < 240) {
        if (dot - 256 > 63u) {  // dot [0..255,320..340]
          ...
          if (dot < 256) {
            ...
            // size.
            //printf("Write pixel to framebuffer : color=%i\n", (int) color); 
            ...
            frame_buffer[scany * 256 + dot] = color * 6000;
            ...
            // static uint16_t P[] = {
                    ...
                    // ~0,    ~328,  ~422,  ~452,  ~482,  58911, 50814, 42620,
                    // 40667, 40729, 48951, 53078, 61238, 44405};
             // int index = color ? palette | color : 0;
             // frame_buffer[scany * 256 + dot] = P[palette_ram[index]];  
             
             //if (palette == 4)             
                //printf("color=%i palette=%i index=%i palette_ram[index]=%i pixel=%i\n", (int)color, (int)palette, (int)index, (int)palette_ram[index], (int)P[palette_ram[index]]);
             
             // if (P[palette_ram[color ? palette | color : 0]] != 0) {
                // printf("palette_ram[%i]=%i  P[palette_ram[color ? palette | color : 0]]=%i\n", (color ? palette | color : 0), (int) palette_ram[color ? palette | color : 0], (int)P[palette_ram[color ? palette | color : 0]]);
             // }
             
             // int i;
             // for (i = 0; i < 64; i++) {
                // printf("palette_ram[%i]=%i\n", i, (int) palette_ram[i]);
             // }
...
// if (frame_buffer[scany * 256 + dot] != 0) {
                      // printf("frame_buffer[%i]=%i\n", (int)(scany * 256 + dot), (int) frame_buffer[scany * 256 + dot]);
